...
module.exports = {
    ...
    insertRecordFromCSV: function (arrayRow) {
        ...
        var tag = arrayRow[19];
        connection.query("INSERT INTO alunni VALUES (?,?,?,?,?,STR_TO_DATE(?,'%d/%m/%Y'),?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [cognome, nome, matricola, cf, sesso, dataDiNascita.split(" ")[0], statoRichiesta, cap, nazionalita, legge_107, legge_104, indirizzo, classe_precedente, annoScolastico, anno, codice_cat, media_voto, condotta, classe_futura,tag], function (err, row) {
            if (err) {
                console.log(err);
            }
        });
